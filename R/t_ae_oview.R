...
t_ae_oview <- function(id,
  ...
  if(!is.null(extra_flag)) {
    df_extra_flags <- data.frame(id = id,
                                 col_by = col_by,
                                 extra_flag,
                                 stringsAsFactors = FALSE)
  }
  ...
  if("serwd" %in% display_id) df_flags$serwd <- factor(if_else(df_flags$aeser == "Y" &
  ...
  #for extra flags
  if(!is.null(extra_flag)){
    df_conv <- apply(df_extra_flags[-c(1, 2)], 2, FUN = function(b){factor(if_else(b == "Y", 1, 0))})
