...
spiderplot_simple <- function(anl, byvar = "USUBJID", days = "TRTDURD",
  ...
  # plotr
  ggplot(data = anl, mapping = aes(x = .data$days, y = .data$mes_value, group = .data$byvar, colour = .data$group_col),
          size = 2, alpha = 1) +
    geom_point(size = 3) + geom_line(size = 2, alpha = 0.7) +
    geom_text(aes(x = .data$days, y = .data$mes_value, label = .data$byvar), data = last_obs, hjust = 0) +
