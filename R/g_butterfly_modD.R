...
#' @param groups 2 column dataframe  of 1/0 (flags of dichotomization values)
...
g_butterfly_modD <- function(category,
                        groups,
                        group_names = NULL,
                        block_count = "# of patients",
                        block_color = NULL,
  ...
  if(!is.null(block_color)){
    pl <- ggplot(NULL, aes(x=y)) +
      ...
      geom_text(data=text_ann1, aes(y = label_ypos, label = bar_count), hjust=0.9) +
